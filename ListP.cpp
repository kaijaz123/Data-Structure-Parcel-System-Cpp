...
class List
{
   ...
   // List operations:
      ...
      void load( string fileName )
        throw( ListException );
        void modification(int index, ListItemType& newItem);
        bool finditem(ListItemType& newItem);
        bool BinarySearch(int & newItem);
...
void List::modification(int index, ListItemType& newItem){
    ListNode *cur = find(index);
    cur->item = newItem;
...
bool List::finditem(ListItemType& newItem){
    bool found = false;
    string item = "";
    ListNode * cur = head;
    for(int i=1; i <=getLength(); i++){
        for(int i =0; i < cur->item.length() ; i++){
            if(isdigit(cur->item[i])){
                item+= cur->item[i];
            }
        }
        if(newItem == item){
...
bool List::BinarySearch(int & newItem){
    bool found = false;
    ListNode * cur = head;
    int total_pos = getLength()/2;
    string newest = to_string(newItem);
    string get = "";
    int i=1;
    while(true){
        if(i == total_pos){
                cur->next;
                string check = cur->item;
                cout << check << endl;
                for(int i=0; i < cur->item.length(); i++){
                    if(isdigit(check[i])){
                        get += check[i];
